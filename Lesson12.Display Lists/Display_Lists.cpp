...
GLuint xloop;                  // Loop for X aixs
GLuint yloop;                  // Loop for Y aixs
...
GLvoid BuildLists()
{
    box = glGenLists(2);                              // Build two lists
...
/******************************************************************************************************************************************/
    ...
    glEnable(GL_LIGHT0);                              // Quick and dirty lighting (Assumed light0 is set up)
...
//    /*
    for (yloop = 1; yloop < 6; ++yloop) {
        for (xloop = 0; xloop < yloop; ++xloop) {
            glLoadIdentity();
            // Position the cube on the screen
            ...
            glColor3fv(boxcol[yloop - 1]);                              // Select bright
...
//    */
    ...
    /*
    for (float yloop = 1.5f; yloop > -1.5f; yloop-=0.1f) {
        for (float xloop = -1.5f; xloop < 1.5f; xloop+=0.05f) {
            glLoadIdentity();
            float a = xloop * xloop + yloop * yloop - 1;
            if (a * a * a - xloop * xloop * yloop * yloop * yloop <= 0.0f) {
                glTranslatef(xloop*13, yloop*13, -50.0f);
                glRotatef(45.0f - (2.0f*yloop) + xrot, 1.0f, 0.0f, 0.0f);   // Tilt the cubes up and down
                glRotatef(45.0f - yrot, 0.0f, 1.0f, 0.0f);                  // Spin cubes left and right
                glColor3fv(boxcol[0]);                              // Select bright
                glCallList(box);                             // Draw the box
                glColor3fv(topcol[0]);
                glCallList(top);
            }
