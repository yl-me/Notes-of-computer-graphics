...
GLvoid BuildFont(GLvoid)
{
    ...
    wglUseFontOutlines(hDC,                           // Select the current DC
        ...
        255,                                          // Number of display lists of build
        base,                                         // Starting display lists
        0.1f,                                         // Deviation from the true outlines
        0.2f,                                         // font thickness in the Z direction
        WGL_FONT_POLYGONS,                            // Use polygons, not lines
        gmf);                                         // Address of buffer to recive data
}
...
GLvoid glPrint(const char* fmt, ...)
{
    ...
    for (unsigned int loop = 0; loop < strlen(text); ++loop) {
        length += gmf[text[loop]].gmfCellIncX;        // Increase length  by each character width
    }
...
int LoadGLTextures()
{
    ...
    if (TextureImage[0] = LoadBMP("1.bmp")) {
        ...
        // GL_S will cover mapping the texture left to right
...
int DrawGLScene(GLvoid)                                  // Here's where we do all the drawing
{
    ...
    glTranslatef(1.1f * float(cos(rot / 116.0f)), 0.8f * float(sin(rot / 120.0f)), -3.0f);

    glRotatef(rot, 1.0f, 0.0f, 0.0f);
    glRotatef(rot * 1.2f, 0.0f, 1.0f, 0.0f);
    glRotatef(rot * 1.4f, 0.0f, 0.0f, 1.0f);
    glTranslatef(-0.35f, -0.35f, 0.1);
