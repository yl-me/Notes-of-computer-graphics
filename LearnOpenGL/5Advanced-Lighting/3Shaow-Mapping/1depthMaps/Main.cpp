...
//	Shader shader("path/base.vert", "path/base.frag");
	Shader simpleDepthShader("path/depthTexture.vert", "path/depthTexture.frag");
	Shader debugDepthQuad("path/debug_quad.vert", "path/debug_quad_depth.frag");
	...
	GLuint depthMap = generateAttachmentTexture(true, false);
...
//		glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
		...
		glm::mat4 lightView = glm::lookAt(glm::vec3(-2.0, 4.0f, -1.0f), glm::vec3(0.0f), glm::vec3(1.0));
...
GLuint generateAttachmentTexture(GLboolean depth, GLboolean stencil)
{
	...
	if (!depth && !stencil) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, attachment_type, 1024, 1024, 0, attachment_type, GL_FLOAT, NULL);
	}
	else {
