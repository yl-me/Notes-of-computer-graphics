...
typedef struct point_3d {               // 3-dimensional
    double x, y, z;
} POINT_3D;

typedef struct bpatch {
    POINT_3D anchors[4][4];             // 4X4 grid of anchors ponits
...
bool showCPoints = true;                 // Toggles displaying the control point grid

int divs = 7;                           // Number of intrapolations (controls poly resolution)
...
GLuint genBezier(BEZIER_PATCH patch, int divs)
{
    ...
    temp[0] = patch.anchors[0][3];              // The first derived curve
    ...
    for (int u = 1; u <= divs; ++u) {
        ...
        temp[0] = Bernstein(py, patch.anchors[0]);        // Calculate new Bezier
        ...
        for (int v = 0; v <= divs; ++v) {
            px = ((float)v) / ((float)divs);
            glTexCoord2f(pyold + a++, px + a++);
            glVertex3d(last[v].x, last[v].y, last[v].z);      // Old point

            last[v] = Bernstein(px, temp);
            glTexCoord2f(py + a++, px + a++);
...
{
    mybezier.anchors[0][0] = makePoint(-0.75f, -0.75f, -0.5f);     // Set the Bezier vertices
...
int InitGL(GLvoid)                                    // All setup for OpenGL goes here
{
    ...
    glClearColor(0.05f, 0.05f, 0.05f, 0.5f);          // Background
    ...
    initBezier();
    LoadGLTexture(&(mybezier.texture), "image.bmp");
...
//******************************************************************************************************************************************/
            ...
            if (keys['A']) rotz -= 0.1f;
            if (keys['D']) rotz += 0.1f;
            if (keys[VK_LEFT]) rotz -= 0.1f;
            if (keys[VK_RIGHT]) rotz += 0.1f;
            if (keys[VK_UP]) {
                divs++;
                mybezier.dlBPatch = genBezier(mybezier, divs);
                keys[VK_UP] = false;
