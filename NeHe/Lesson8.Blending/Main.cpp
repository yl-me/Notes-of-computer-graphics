...
/******************************************************************************************************************************************/
    ...
    glLightfv(GL_LIGHT1, GL_POSITION, LightPosition);    // Steup the position light
...
int DrawGLScene(GLvoid)                                  // Here's where we do all the drawing
{
     ...
     */
    glTranslatef(0.0f, 0.0f, z);                         // Move into the screen 5 units
    glRotatef(xrot, 1.0f, 0.0f, 0.0f);
    glRotatef(yrot, 0.0f, 1.0f, 0.0f);

    glBindTexture(GL_TEXTURE_2D, texture[filter]);            // Select a texture baseds on filter
    ...
    int nCmdShow)                                         // Window show state
{
    ...
    while (!done) {                                       // Loop that runs until donw = TRUE
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {   // Is there a message wating
            ...
            if (keys[VK_SUBTRACT]) {                      //VK_PRIOR
                z -= 0.005f;
            }
            if (keys[VK_ADD]) {                           // VK_NEXT
                z += 0.005f;
            }
            if (keys[VK_UP]) {
                xspeed -= 0.0005f;
