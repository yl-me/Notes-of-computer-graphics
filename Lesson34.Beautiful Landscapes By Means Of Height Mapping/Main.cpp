...
#define MAP_SIZE 1024                // Size of height map
...
bool bRender = TRUE;    // Polygon flag set to TURE by default
BYTE g_HeightMap[MAP_SIZE * MAP_SIZE];    // Map data
...
void LoadRawFile(LPSTR strName, int nSize, BYTE* pHeightMap)
{
    ...
    // Reading in '1', and the size is width * height
...
void SetVertexColor(BYTE* pHeightMap, int x, int y)  // This sets the color values for a particular index
{
    if (!pHeightMap)
        return;
    float fcolor = -0.15 + (Height(pHeightMap, x, y) / 256.0f);
...
void RenderHeightMap(BYTE pHeightMap[])             // This renders the height map as quads
{
    ...
    for (int X = 0; X < (MAP_SIZE - STEP_SIZE); X += STEP_SIZE) {
        for (int Y = 0; Y < (MAP_SIZE - STEP_SIZE); Y += STEP_SIZE) {
            // Get the value for the bottom left vertex
            x = X;
            ...
            // Get the value for the top left vertex
            x = X;
            ...
            // Get the value for the top right vertex
            x = X + STEP_SIZE;
            ...
            // Get the value for the nottom right vertex
            x = X + STEP_SIZE;
    ...
    glEnd();
    glColor4f(1.0f, 1.0f, 1.0f, 1.0f);        // Reset
}
