...
int InitGL(GLvoid)                                    // All setup for OpenGL goes here
{
    ...
    glDepthFunc(GL_LEQUAL);                           // The typr of depth test to do
...
int DrawGLScene(GLvoid)                                  // Here's where we do all the drawing
{
    ...
    /*
     *  When you do a glLoadIdentity() what you are doing is moving back to 
     *  the center of the screen with the X axis(轴) running left to right, 
    ...
    glRotatef(rtri, 0.9f, 0.6f, 0.9f);                   // Rotate the triangle on the Y axis (new)

//    glColor3f(0.5f, 0.5f, 1.0f);                         // Set the color to a nice blue shade
    glBegin(GL_TRIANGLES);                               // Drawing using traingles
    ...
    glEnd();                                             // Finished drawing the traingles
    ...
    glLoadIdentity();                                    // Reset (new)
    glTranslatef(1.5f, 0.0f, -6.0f);                      // Move left 3 units
    glRotatef(rquad, 0.5f, 0.5f, 0.5f);                  // Rotate the quad on the x axis (new)
    ...
    int nCmdShow)                                         // Window show state
{
    ...
    if (!CreateGLWindow("Rotation", 640, 480, 16, fullscreen)) {  // (Modified)
    ...
    while (!done) {                                       // Loop that runs until donw = TRUE
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {   // Is there a message wating
            ...
            if (keys[VK_F1]) {                            // Is F1 being pressed
                ...
                if (!CreateGLWindow("Rotation", 640, 480, 16, fullscreen)) {
                    return 0;                             // Quit if window was not create
                }
            }
        }
