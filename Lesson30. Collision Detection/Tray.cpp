...
// Line between two  point OR point and direction
...
bool TRay::adjacentPoints(const TRay& ray, TVector& point1, TVector& point2) const
{
    if (isValid() && ray.isValid()) {
        ...
        TVector tv;                    // Temporary vector to enable of optimised routines
...
// Streaming
std::ostream& TRay::write(std::ostream& out) const
{
    return isValid() ? out << "(p=" << _P << ",V=" << _V << ")" : out << "Invalid";
}

std::istream& TRay::read(std::istream& in)
{
    char buf[20], ch;

    in >> ch;
    if (ch != 'I')
        in >> ch >> ch >> _P >> ch >> ch >> _V;
    else {
        *this = TRay();
        in.get(buf, 20, 'd');
    }
